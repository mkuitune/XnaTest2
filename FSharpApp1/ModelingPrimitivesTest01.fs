...
type AssetManager(manager: ContentManager) = 
    ...
    member this.LoadTexture2DCached(name:string) =
        if tex2d.ContainsKey(name) |> not then 
            tex2d.[name] <- manager.Load<Texture2D>(name)
        tex2d.[name]
    member this.LoadTexture2D(name:string) = this.LoadTexture2DCached(name) //manager.Load<Texture2D>(name)
 (*
type HoverState =
    |NonHovering
    |Hovering
...
type ScreenApplication(g:GraphicsDevice, cont:ContentManager) = 
    ...
    member this.AssetManager = assetManager
    member this.Draw(time:GameTime, device:GraphicsDevice) = 
...
type ModelingTest02() as this = 
    ...
    member this.ConstructScene(app:ScreenApplication) = 
        dbgmsg("ConstructScene")
        let tex1 = base.Content.Load<Texture2D>("icontest-ball")
        //let tex2 = app.AssetManager.LoadTexture2D("icontest-ball")

        //let tex5 = base.Content.Load<Texture2D>("ship1")
        //let tex3 = base.Content.Load<Texture2D>("icontest-background")
        //let tex4 = app.AssetManager.LoadTexture2D("icontest-background")
    ...
    override this.LoadContent() = 
        dbgmsg("LoadContent")
        //let tex = base.Content.Load<Texture2D>("icontest-ball")
        //let tex5 = base.Content.Load<Texture2D>("ship1")
        //let tex3 = base.Content.Load<Texture2D>("icontest-background")
        d <-Some(ScreenApplication(this.GraphicsDevice, base.Content))
        match d with
        | Some(app) -> this.ConstructScene(app)
        | _ -> ()

    override this.UnloadContent() =
        dbgmsg("UnloadContent")
